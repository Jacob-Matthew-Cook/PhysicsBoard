...
Spool4.prototype.setNiceViewCenter = function (gameGraphics, canvas) {
    gameGraphics.PTM = 1000;
    setViewCenterWorld(gameGraphics, canvas, this.position, true);
}

Spool4.prototype.setup = function (gameState, gameGraphics) {
    ...
    shape1.fillStyle = function () { return 'rgba(193, 154, 107, 0.7)'; };
    shape1.strokeStyle = function () { return 'rgb(0,0,0)'; };
    shape1.draw = function (context) {
        //draw Outermost Circle
        drawCircle(context, this.tc, 4*this.r);

        //draw Spokes
        drawLine(context, cp.v.rotate(cp.v(1, 0), cp.v.mult(this.body.rot, 2*this.r)).add(this.tc) , cp.v.rotate(cp.v(1, 0), cp.v.mult(this.body.rot, 4*this.r)).add(this.tc));
        drawLine(context, cp.v.rotate(cp.v(0, 1), cp.v.mult(this.body.rot, 2*this.r)).add(this.tc), cp.v.rotate(cp.v(0, 1), cp.v.mult(this.body.rot, 4*this.r)).add(this.tc));
        drawLine(context, cp.v.rotate(cp.v(-1, 0), cp.v.mult(this.body.rot, 2*this.r)).add(this.tc), cp.v.rotate(cp.v(-1, 0), cp.v.mult(this.body.rot, 4*this.r)).add(this.tc));
        drawLine(context, cp.v.rotate(cp.v(0, -1), cp.v.mult(this.body.rot, 2*this.r)).add(this.tc), cp.v.rotate(cp.v(0, -1), cp.v.mult(this.body.rot, 4*this.r)).add(this.tc));
        ...
        drawLine(context, cp.v(0, -2 * this.r).add(this.tc), cp.v(0, -2 * this.r).add(this.tc).add(cp.v(-2*this.body.p.x +80, 0)));
        context.lineWidth = 0.003;
        context.strokeStyle = 'rgba(213, 174, 127, 1)';
        drawLine(context, cp.v(0, -2 * this.r).add(this.tc), cp.v(0, -6 * this.r).add(this.tc).add(cp.v(-2*this.body.p.x +80, 0)));
        context.restore();
        drawCircle(context, this.tc, 2 * this.r);
...
Spool4.prototype.step = function (gameState, gameDygraph, dataTimeStep, graphTimeStep) {
    var force = cp.v(2*gameState.stepTime / (500), 0);
    var r = cp.v(0, -2*this.radius);
...
Spool4.prototype.initializeDygraph = function (gameState, gameDygraph, graphID) {
...
Spool4.prototype.updateCurrentParameters = function (t1, f1, f2, f3) {
    var time = document.getElementById('running-time');
    var forceUp = document.getElementById('spool-right');
...
Spool4.prototype.addToDygraph = function (gameState, gameDygraph, dataTimeStep, graphTimeStep, x, y, z) {
...
Spool4.prototype.updateInitialParameters = function () {
    ...
    table.innerHTML =
        ...
        '<td id=\"length-initial-1\">Right</td>' +
        '</tr>' +
        //body1
        '<tr class=\'both-pendulums-init\'>' +
        '<td>Mass</td>' +
